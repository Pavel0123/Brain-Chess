...
exports.startGame3min = functions.database.ref("/queue/10 min")
    .onUpdate(async (result, context) => {
      ...
      if (player1 && player2) {
        ...
        database().ref("queue/10 min/" + player1.uid).remove();
        database().ref("queue/10 min/" + player2.uid).remove();
        ...
        //create game
         await database().ref("game/"+ Date.now())
            .update({playerWhite: player1.uid, playerBlack: player2.uid,
             deck: array, timeWhite: 600, timeBlack: 600, startTurn: Math.floor(Date.now() / 1000)});
      }
    });


    exports.startGame10min = functions.database.ref("/queue/3 min")
    .onUpdate(async (result, context) => {
      const data = result.after.val();
      let player1;
      let player2;
      ...
      if (player1 && player2) {
        ...
        //create game
         await database().ref("game/"+ Date.now())
            .update({playerWhite: player1.uid, playerBlack: player2.uid,
             deck: array, timeWhite: 180, timeBlack: 180, startTurn: Math.floor(Date.now() / 1000)});
      }
    });
